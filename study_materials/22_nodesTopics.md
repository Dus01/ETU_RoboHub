...
Чтобы легче понять **узлы**, давайте представим наш узел камеры как фотографа, который начинает работать в офисе журнала. В первый день своей работы он делает несколько фотографий и уведомляет своего босса, что он оставит эти фотографии на определенном столе в офисе. Босс благодарит его за уведомление.

В нашей истории Босс представляет **Мастера ROS**, который всегда должен быть у вас при использовании ROS. Мастер ROS предоставляет услуги именования и регистрации для остальных узлов в системе ROS. Он отслеживает публикаторов и подписчиков **топиков**. Мастер ROS вызывается с помощью команды `roscore`. **Одновременно должен работать только один Мастер ROS**. Если вы попытаетесь вызвать другой, возникнет ошибка. Фотограф представляет **Узел ROS**, который всегда должен регистрироваться на Мастере при запуске, подобно тому, как Фотограф уведомляет своего Босса.
...
Начнем, чтобы что-то запустить **узел**, нужно воспользоваться утилитой запуска узла `rosrun`, которая имеет следующее описание:
...
Вернемся к нашему фотографу. Фотограф обменивается сообщениями, которые относятся к типу `sensor_msgs`. То, что фотограф хочет нам сказать называется **ROS Publisher**.

Давайте взглянем на код, в котором фотограф будет приветствовать нас каждую секунду
...
А что такое **ROS Subscriber**? Давайте теперь представим ситуацию, что в нашей истории есть еще один человек, редактор, который хочет начать работать в одной команде с фотографом - брать у него интервью и готовые снимки.
Что ему нужно сделать? 
Первоначально ему нужно уведомить Босса (master) о командной работе, а дальше, если фотограф идет на контакт, то начать с ним взаимодействовать.

<p align="center">
    <img src="../assets/ros/ros_subscriber.avif" width=500>
</p>

После уведомления Босса и фотографа все начинают знать друг о друге, и теперь они просто обмениваются сообщениями друг с другом и принимают их друг у друга соответственно.

<p align="center">
    <img src="../assets/ros/ros_sub.avif" width=500>
</p>

Тут важно запомнить, что, чтобы фотограф и редактор могли корректно работать в команде - они должны наладить контакт по одному источнику - **топику**. Давайте теперь напишем узел, который позволит редактору слушать то, что говорит фотограф.
