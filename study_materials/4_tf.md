...
- [Всё начинается с системы координат](#всё-начинается-с-системы-координат)
- [Попытка отобразить информацию о роботе - не пытка](#попытка-отобразить-информацию-о-роботе---не-пытка)
- [TF 101](#tf-101)
...
Начтем мы с системы координат робота.

## Попытка отобразить информацию о роботе - не пытка
...
```xml
<node pkg="tf2_ros" type="static_transform_publisher" name="map_2_panda_link0" args="0 0 0 0 0 0 map panda_link0" />
```
